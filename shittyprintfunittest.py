...
UNITTEST = {
	...
	# "test_int_random_13": ["[%-54.12d]", 987823],
	...
	"test_all_mixed_03": ["qwerty [%1.2*s], [%087c], [%21.62d], [%087u],  [%-*x], [%-1.*d],", 3, "qwerty", 'X', 12, 45678965, -6, 14, 456, 45],
	"test_all_mixed_03": ["%% qwerty [%1.*s], [%87c], [%21.62d], [%087u],  [%-*x], [%-1.*d],", 3, "qwerty", 'X', 12, 45678965, -6, 14, 456, 45],
}
...
def compile_main(filename, printf_args):
    ...
    ret = os.system(
        "gcc {filename} -Wall -Werror -Wextra srcs/*/*.o srcs/*.o -Iincludes libft/libft.a -o {out}".format(filename=filenamec, out=filename))
    ret += os.system(
        "gcc {filename} -Wall -Werror -Wextra srcs/*/*.o srcs/*.o -Iincludes libft/libft.a -o {out}".format(filename=temoin, out=filename + '_temoin'))
    if ret:
